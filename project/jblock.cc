...
void Jblock::rotateCW(std::vector<std::shared_ptr<Row>> theGrid) {
    ...
    if (pos = FACEUP) {
        // Set the new cells
        (theGrid[x]->(*rowData[y + 2])).setBlock(blockType::J);

        (theGrid[x1 - 1]->(*rowData[y1 + 1])).setBlock(blockType::J);

        (theGrid[x2 - 2]->(*rowData[y2])).setBlock(blockType::J);

        (theGrid[x3 + 1]->(*rowData[y3 + 1])).setBlock(blockType::J);
        ...
        // Set the new cells
        (theGrid[x + 2]->(*rowData[y])).setBlock(blockType::J);

        (theGrid[x1 + 1]->(*rowData[y1 + 1])).setBlock(blockType::J);

        (theGrid[x2]->(*rowData[y2 + 2])).setBlock(blockType::J);
        ...
        // Set the new cells
        (theGrid[x]->(*rowData[y])).setBlock(blockType::J);

        (theGrid[x1 + 1]->(*rowData[y1 - 1])).setBlock(blockType::J);

        (theGrid[x2 + 2]->(*rowData[y2 - 2])).setBlock(blockType::J);
        ...
        // Set the new cells
        (theGrid[x - 2]->(*rowData[y + 2])).setBlock(blockType::J);
...
void Jblock::rotateCCW(std::vector<std::shared_ptr<Row>> theGrid) {
    rotateCW(std::vector<std::shared_ptr<Row>> theGrid);
    rotateCW(std::vector<std::shared_ptr<Row>> theGrid);
    rotateCW(std::vector<std::shared_ptr<Row>> theGrid);
}

void Jblock::moveDown(std::vector<std::shared_ptr<Row>> theGrid) {
    // Get the coordinates for all the cells the block occupies
    // Get the coordinates for all the cells the block occupies
    int x = (*occupiedCells[0]).getCol(); // Top/rightmost cell
    int y = (*occupiedCells[0]).getRow();
    ...
    // Set each new cell
    (theGrid[x]->(*rowData[y - 1])).setBlock(blockType::J);

    (theGrid[x1]->(*rowData[y1 - 1])).setBlock(blockType::J);
    
    (theGrid[x2]->(*rowData[y2 - 1])).setBlock(blockType::J);

    (theGrid[x3]->(*rowData[y3 - 1])).setBlock(blockType::J);
...
void Jblock::moveLeft(std::vector<std::shared_ptr<Row>> theGrid) {
    // Get the coordinates for all the cells the block occupies
    int x = (*occupiedCells[0]).getCol(); // Top/rightmost cell
    int y = (*occupiedCells[0]).getRow();
    ...
    // Set each new cell
    (theGrid[x - 1]->(*rowData[y])).setBlock(blockType::J);

    (theGrid[x1 - 1]->(*rowData[y1])).setBlock(blockType::J);
    
    (theGrid[x2 - 1]->(*rowData[y2])).setBlock(blockType::J);

    (theGrid[x3 - 1]->(*rowData[y3])).setBlock(blockType::J);
...
void Jblock::moveRight(std::vector<std::shared_ptr<Row>> theGrid) {
    // Get the coordinates for all the cells the block occupies
    int x = (*occupiedCells[0]).getCol(); // Top/rightmost cell
    int y = (*occupiedCells[0]).getRow();
